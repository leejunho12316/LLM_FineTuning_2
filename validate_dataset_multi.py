...
def _extract_all_create_statements(instruction: str) -> list[str]:
    """instruction 안의 CREATE TABLE 문을 모두(멀티 테이블 대응) 추출한다."""
    return [s.strip() for s in re.findall(r"(CREATE TABLE.*?;)", instruction, re.S)]


def _extract_all_insert_statements(instruction: str) -> list[str]:
    """instruction 안의 INSERT INTO 문을 모두(멀티 테이블 대응) 추출한다."""
    return [s.strip() for s in re.findall(r"(INSERT INTO.*?;)", instruction, re.S)]
...
def _validate_instruction(item, idx, report) -> dict:
    ...
    create_sqls = _extract_all_create_statements(instr)
    insert_sqls = _extract_all_insert_statements(instr)

    # 각 CREATE를 파싱해 테이블명 -> create_info 매핑 구성
    create_infos = {}
    for c_sql in create_sqls:
        c_info = _parse_create(c_sql)
        if c_info is None:
            _add(report, idx, "instruction 값", "CREATE TABLE 문 파싱 가능 여부", False,
                 detail="sqlglot 파싱 실패")
        else:
            create_infos[c_info["table_name"]] = c_info

    # output 컬럼 검증용: 모든 테이블 컬럼의 합집합
    all_ddl_columns = set()
    for c_info in create_infos.values():
        all_ddl_columns |= set(c_info["columns"].keys())
    ctx["all_ddl_columns"] = all_ddl_columns
    # 하위호환: 단일 테이블 코드 경로를 위해 첫 테이블 create_info도 보관
    ctx["create_info"] = next(iter(create_infos.values()), None)

    if not insert_sqls:
        # INSERT문이 없는 경우(VALUES 0개 샘플링)는 정상 케이스
        return ctx

    # 각 INSERT를 해당 테이블의 CREATE와 매칭해 검증
    for insert_sql in insert_sqls:
        ...
        create_info = create_infos.get(insert_info["table_name"])
        if create_info is None:
            _add(report, idx, "instruction(INSERT)", "CREATE/INSERT 테이블명 일치", False,
                 detail=f"INSERT 테이블={insert_info['table_name']}가 CREATE에 없음")
            continue

        _validate_one_insert(create_info, insert_info, idx, report)

    return ctx


def _validate_one_insert(create_info, insert_info, idx, report):
    """단일 (CREATE, INSERT) 쌍에 대한 컬럼/타입/NULL/PK 검증."""
    # 1) 테이블명 일치
...
def _validate_output(item, idx, report, ddl_columns, conn_factory=None, llm_sql=None) -> str | None:
    output = item["output"]
...
def _build_row_sqlite_conn(instruction: str):
    """instruction에 들어있는 모든 CREATE TABLE(+INSERT) 샘플로 임시 in-memory DB를 구성(멀티 테이블 대응)"""
    create_sqls = _extract_all_create_statements(instruction)
    insert_sqls = _extract_all_insert_statements(instruction)
    if not create_sqls:
        return None
...
def validate_dataset(data: list, llm_sql=None, _convert_to_sqlite=None, check_execution: bool = True) -> list[dict]:
    ...
    for idx, item in enumerate(data):
        ...
        _validate_output(item, idx, report, ctx.get("all_ddl_columns", set()),
